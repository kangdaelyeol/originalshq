/**
 * 엑셀 다운로드(excel-export-modal.tsx)의 실제 ".xlsx 워크시트를 어떻게 채울지"
 * 로직 — 전부 ExcelJS 워크북/워크시트 객체와 평 데이터만 다루는 순수 함수라
 * React 상태와 무관하다. 모달 컴포넌트는 이 모듈이 내보내는 계획/쓰기 함수를
 * 호출하기만 하는 얇은 오케스트레이터로 남는다.
 */
...
export type ChannelSelector = 'combined' | ChannelKey
...
export function writeTotalSheet(
  wb: ExcelJS.Workbook,
  total: {
...
export function writeCampaignSheet(
  wb: ExcelJS.Workbook,
  sheetName: string,
 ...
 * 애드셋만 추려서 넘긴다. */
export function writeAdsetSheet(
  wb: ExcelJS.Workbook,
  sheetName: string,
  adsets: readonly (CombinedAdset & { channelLabel: string })[],
  channelSelector: ChannelSelector,
